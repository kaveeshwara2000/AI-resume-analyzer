  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 flex-1 flex flex-col space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        ...
        <Link
          to="/analyze"
          className="inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-md hover:bg-indigo-500 transition-colors space-x-2"
        >
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        {/* Stat 1 */}
        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800/80 dark:bg-slate-900 transition-all hover:shadow-md">
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800/80 dark:bg-slate-900 transition-colors flex-1 flex flex-col min-h-[400px]">
        ...
        <div className="p-6 border-b border-slate-200/80 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-xs">
              {searchQuery ? 'No results matches your query.' : 'Upload and run your first resume analysis report to see details here.'}
            </p>
            {!searchQuery && (
              <Link
                to="/analyze"
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        item.resumeScore >= 80 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400' :
                        item.resumeScore >= 60 ? 'bg-amber-50 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400' :
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-500 dark:text-slate-400 flex items-center space-x-1.5 pt-5">
      ...
      {/* Analysis Details Slide-over Modal */}
      {selectedAnalysis && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-fade-in">
            {/* Modal Header */}
            <div className="p-6 border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-slate-950 dark:text-white">Analysis Details</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm leading-relaxed text-slate-700 dark:text-slate-300">
              
              {/* Scores Grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-indigo-50/50 dark:bg-indigo-950/20 rounded-2xl p-5 border border-indigo-100/50 dark:border-indigo-900/30 flex items-center justify-between">
                  <div>
                ...
                <div className="bg-violet-50/50 dark:bg-violet-950/20 rounded-2xl p-5 border border-violet-100/50 dark:border-violet-900/30 flex items-center justify-between">
                  <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedAnalysis.matchedSkills?.length > 0 ? (
                      selectedAnalysis.matchedSkills.map((skill, index) => (
                        <span key={index} className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-400 border border-emerald-250 dark:border-emerald-900/30">
                          {skill}
                        </span>
                      ))
                    ) : (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedAnalysis.missingSkills?.length > 0 ? (
                      selectedAnalysis.missingSkills.map((skill, index) => (
                        <span key={index} className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-rose-50 text-rose-800 dark:bg-rose-950/30 dark:text-rose-400 border border-rose-255 dark:border-rose-900/30">
                          {skill}
                        </span>
                      ))
                    ) : (
                  ...
                  {selectedAnalysis.improvementSuggestions?.length > 0 ? (
                    selectedAnalysis.improvementSuggestions.map((suggestion, index) => (
                      <li key={index} className="flex items-start space-x-2">
                        ...
                        <span className="text-slate-700 dark:text-slate-50">{suggestion}</span>
                      </li>
                    ))
                  ) : (
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-slate-200/80 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 flex justify-end">
              <button
                onClick={() => setSelectedAnalysis(null)}
                className="px-5 py-2.5 rounded-xl bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-sm font-semibold text-slate-800 dark:text-slate-200 transition-colors"
              >
                Close details
              </button>
            </div>
